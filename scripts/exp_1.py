...
if __name__ == "__main__":
    ...
    for image in images:
        ...
        print("\n")
        print(f"The total area of the {image.name} plant is {round(image.plant_area_in2, 2)} inches^2.")
        print(f"The {image.name} plant is {round(image.plant_length_in, 2)} inches by {round(image.plant_width_in, 2)} inches @ max.")
        print(f"The {image.name} plant density {round(image.plant_area_in2 / (image.plant_width_in * image.plant_length_in) * 100, 2)}%.")
        print("\n")
